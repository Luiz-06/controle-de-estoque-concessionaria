...
BEGIN
    ...
    SELECT LC.COD_LOJA INTO v_cod_loja_funcionario
    FROM FUNCIONARIO F
    ...
    IF v_cod_loja_funcionario <> v_cod_loja_carro THEN
        RAISE EXCEPTION 'O funcionário (da loja %) só pode vender carros da sua própria loja (tentativa de vender da loja %).',
            v_cod_loja_funcionario, v_cod_loja_carro;
    END IF;
